...
module.exports.addMovieDB = (movie, callback) => {
  connection.query(`INSERT INTO movies (title, poster, year, genre, type, rated, watched) VALUES ("${movie.title}", "${movie.poster}", "${movie.year}", "${movie.genre}", "${movie.type}", "${movie.rated}", ${movie.watched})`, callback);
}

module.exports.getMovieDB = (callback) => {
  connection.query(`SELECT id, title, poster, year, genre, type, rated, watched, created, updated FROM movies`, callback);
}
